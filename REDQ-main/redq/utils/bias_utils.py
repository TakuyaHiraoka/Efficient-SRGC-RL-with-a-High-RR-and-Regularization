...
def get_mc_return_with_entropy_on_reset(bias_eval_env, agent, max_ep_len, alpha, gamma, n_mc_eval, n_mc_cutoff,
    ...
    final_obs_list = []
    final_act_list = []
    final_goal_list = [] # for goal conditioned env. TH20230817
    while final_mc_list.shape[0] < n_mc_eval:
        ...
        goal_list = [] # for goal conditioned env. TH20230817
        ...
        for i_step in range(max_ep_len):  # run an episode
            ...
            # store obs and goal to different lists. TH20230817
            if is_goal_conditioned_env:
                obs_list.append(o["observation"])
                goal_list.append(o["desired_goal"])
            else:
                obs_list.append(o)
            act_list.append(a)
        ...
        final_goal_list += goal_list[:n_mc_cutoff] # add TH 202430817
        final_obs_list += obs_list[:n_mc_cutoff]
        final_act_list += act_list[:n_mc_cutoff]
    return final_mc_list, final_mc_entropy_list, np.array(final_obs_list), np.array(final_act_list), np.array(final_goal_list) # add goal list 20230817
...
def log_bias_evaluation(bias_eval_env, agent, logger, max_ep_len, alpha, gamma, n_mc_eval, n_mc_cutoff,
    ...
    final_mc_list, final_mc_entropy_list, final_obs_list, final_act_list, final_goal_list = get_mc_return_with_entropy_on_reset(bias_eval_env,
                                                                                                               agent,
                                                                                                               max_ep_len,
                                                                                                               alpha, gamma,
                                                                                                               n_mc_eval,
                                                                                                               n_mc_cutoff,
                                                                                                               # TH 20230817
                                                                                                               is_goal_conditioned_env
                                                                                                               )
    logger.store(MCDisRet=final_mc_list)
    logger.store(MCDisRetEnt=final_mc_entropy_list)
    if is_goal_conditioned_env:
        obs_tensor = agent._get_obs_goal_torch_tensor(np_obs=final_obs_list, np_dgoal=final_goal_list, unsqueeze=False)
    else:
    ...
    # switch return base in accordance with boundq flag. TH20230817
    if boundq:
    ...
    if boundq:
        final_mc_entropy_list_normalize_base = final_mc_list.copy()
    else:
